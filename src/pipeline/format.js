...
export default function formatEvent(attributes = {}) {
  ...
  icsFormat += `DTSTART${start && start.length == 3 ? ";VALUE=DATE" : ""}${setTimezone(timezone)}:${setDate(start, startType)}\r\n`
  ...
  if (!(_.isEqual(start, end) && end && end.length == 3)) {
    if (end && end.length == 3) {
      ...
      icsFormat += `DTEND${setTimezone(timezone)}:${setDate(end, startType)}\r\n`;
    }
  }
